...
## 整體流程:問卷 → 訪談綱要 → 面談測試人員 → 筆記補完 → 組長訪談前彙整 → 面談組長 → 資料缺口檢查
...
6. 拿這份清單面談組長,組長筆記同樣照範本記、存進 `results/interview-notes/`。
7. 三份結構化筆記(測試人員 ×2 + 組長)到齊後,觸發 `gap-check` skill 跑 go/no-go 資料缺口檢查:(夠)進合併分析 /(不夠)先精準補訪再回來。最終根因分析(合併分析)仍不在本工具範圍,留待另支處理。
...
- 輸入:`results/interview-notes/{稱呼}_訪談紀錄.md`(測試人員與組長面談紀錄,格式=`docs/訪談筆記範本.md` 填好的樣子;題號為指標、非題目全文,需搭配同人訪綱對照)。
- 輸出:`results/lead-prep/_組長訪談前彙整.md`(單一彙整檔)。
- 輸出:`results/gap-check/_資料缺口檢查報告.md`(單一報告檔;`gap-check` 讀 `results/interview-notes/` 三份筆記 + `results/lead-prep/` 彙整清單後產出)。
...
- 位置:`.claude/skills/gap-check/`(專案層級,隨 git 版控)。
- 觸發:對話說「資料缺口檢查」或打 `/gap-check`(預設掃 `results/interview-notes/` + 讀 `results/lead-prep/_組長訪談前彙整.md`)。
- 檢查規則正本:`.claude/skills/gap-check/reference/缺口檢查提示詞.md`(唯一真實來源,改規則只改這份)。
- 用途:三份訪談筆記(測試人員 ×2 + 組長)到齊後、合併分析前的 go/no-go 驗收關卡——跑六道檢查(兩大缺口/彙整提問vs組長回答/上下落差/太表層/全局漏問/未提及彙總),只盤點缺口、不腦補,產出補訪待辦與 go/no-go 總結論,作為下一支合併分析的綠燈/紅燈。
- 已知限制:只盤點缺口、產 go/no-go,**不**做根因分析;需三份筆記 + 彙整清單齊備才能完整跑(缺組長筆記或彙整清單時會停下請補)。
